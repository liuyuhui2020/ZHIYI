...
pytestmark = [
    ...
    pytest.mark.skipif(
        ...
        or not os.getenv("ZHIYI_ANTHROPIC_SMOKE_MODEL"),
...
@pytest.mark.asyncio
async def test_anthropic_one_request_smoke() -> None:
    reference = SecretReference("ANTHROPIC_API_KEY")
    ...
    target = ModelTarget(
        ...
        model_id=os.environ["ZHIYI_ANTHROPIC_SMOKE_MODEL"],
        credential=reference,
        capabilities=ModelCapabilityProfile(),
        limits=ModelLimits(attempt_timeout_seconds=10, max_retries=0),
    )
    request = ModelRequest(
        request_id="anthropic-online-smoke",
