"""Provider-neutral behavioral contract for RunRepository implementations."""
...
from zhiyi.application.ports.run_repository import CommandReceipt, RunRepository
...
async def commit_create(repository: RunRepository) -> tuple[Run, CommandReceipt]:
    mutation = new_run()
...
class RunRepositoryContract:
    """Inherited unchanged by every repository adapter contract suite."""
    ...
    async def test_create_load_and_ordered_event_read_are_atomic(
        ...
        assert found.receipt == command_receipt
    ...
    async def test_cursor_tail_and_all_supported_page_limits(
        self,
        repository: RunRepository,
    ) -> None:
        created, _ = await commit_create(repository)
        ...
        first_page = await repository.list_events(created.tenant_id, created.run_id, limit=1)
        ...
        assert first_page + second_page == new_run().events + started.events
