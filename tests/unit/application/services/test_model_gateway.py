...
@pytest.mark.asyncio
async def test_complete_accepts_multiple_verified_tool_calls() -> None:
    provider = FakeProvider(
        FakeScript(
            completions=(
                ProviderResponse(
    ...
    response = await gateway.complete(ModelRoute(tool_target()), tool_request())
...
@pytest.mark.asyncio
async def test_stream_assembles_multiple_tool_calls_by_index() -> None:
    provider = FakeProvider(
        FakeScript(
            streams=(
                (
                    ProviderChunk(
                        tool_call_delta=ToolCallDelta(
                            ...
                            id="call-2",
                            name="lookup_weather",
                            arguments_fragment='{"city":"Beijing"}',
                        )
                    ),
                    ProviderChunk(
                        tool_call_delta=ToolCallDelta(
                            ...
                            id="call-1",
                            name="lookup_weather",
                            arguments_fragment='{"city":"Shanghai"}',
                        )
                    ),
                    ProviderChunk(finish_reason=FinishReason.TOOL_CALLS),
                ),
...
@pytest.mark.asyncio
async def test_secret_resolution_failure_maps_to_authentication_before_provider_call() -> None:
    provider = CredentialRecordingProvider("secure")
    reference = SecretReference("SECURE_KEY")
    gateway = DefaultModelGateway(
        providers=(provider,),
        token_estimator=ConservativeTokenEstimator(),
    ...
    secure_target = ModelTarget(
        ...
        capabilities=target().capabilities,
        limits=ModelLimits(max_retries=3),
    )

    with pytest.raises(ModelError) as caught:
        await gateway.complete(ModelRoute(secure_target), request())
...
@pytest.mark.asyncio
async def test_missing_secret_provider_fails_before_provider_call() -> None:
    provider = CredentialRecordingProvider("secure")
    reference = SecretReference("SECURE_KEY")
    gateway = DefaultModelGateway(
        providers=(provider,),
