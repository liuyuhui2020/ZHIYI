...
每次转换使用预期版本或行锁，重复命令返回当前状态。`waiting_resolution` 需要具有权限的 Operator 对未知 Tool 结果执行确认、补录或终止。
...
M1 使用专用的 `langchain-openai` 和 `langchain-anthropic` 包。避免依赖不需要的 `langchain-community` 集成。
...
- Structured Output 使用 Pydantic 校验，解析失败次数受预算限制。
...
- 状态机、预算、Context 选择、Policy、Memory Policy 和错误分类。
- 使用确定性 Fake Model，不依赖在线模型。

### 21.2 契约测试

- OpenAI/Anthropic消息、Tool Calling、Structured Output、Usage 和错误映射。
