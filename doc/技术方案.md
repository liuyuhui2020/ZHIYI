...
> 状态：候选架构基线；Model Gateway 独立切片已实现，完整 Runtime 尚未实现
> 版本：v0.1
> 更新日期：2026-08-24
...
实现 ModelProvider 并显式注册即可，不需要修改平台请求/响应契约。当前实现不包含
Agent Graph、Run 持久化、API、Worker、Checkpoint、Langfuse 或 Tool 执行。
