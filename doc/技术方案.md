...
> 状态：候选架构基线；Model Gateway、Run Lifecycle Kernel 与 PostgreSQL RunRepository 已实现，完整 Runtime 尚未实现
> 版本：v0.1
> 更新日期：2026-08-25
...
Alembic `alembic_version` 管迁移图，应用自有的 `zhiyi_schema_compatibility` 管可接受读写
契约，当前均为首版/contract 1。迁移只允许独立 Job 执行 `alembic upgrade head`；应用
启动不调用 `create_all`、stamp、upgrade 或修复。首个 downgrade 会逆序删除全部本特性数据，
只允许身份已核验的空库/一次性数据库。保留数据的回退边界是逻辑备份恢复到新数据库后切换，
不是在原生产库执行 downgrade。

本切片不连接 Model Gateway、LangGraph、Tool、Worker、租约、Checkpoint、API 或 SSE。
