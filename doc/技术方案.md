...
> 状态：候选架构基线；Model Gateway、Run Lifecycle Kernel、PostgreSQL RunRepository 与 Worker Lease Kernel 已实现，完整 Runtime 尚未实现
> 版本：v0.1
> 更新日期：2026-08-27
...
内存仓储仅用于本地执行和契约验证。PostgreSQL adapter 已复用同一端口与契约，使用
SQLAlchemy Core async + Psycopg 3，并把 `runs`、`run_events`、
`run_command_receipts` 作为三类产品事实：command receipt 的 `(tenant_id, command_id)`
立即唯一约束是事务首个仲裁点；获胜事务再锁定 Run、校验 expected_version 和共享不变量，
最后写零或一个 Event 并显式提交。不同状态变更在同一版本至多一个成功；合法零事件命令只
写 receipt，不推进 Run version。

事件 `event_id` 全局唯一，但所有读取和 receipt 反查仍同时带 tenant_id/run_id，不能把全局
标识当授权。事件 sequence 与 Run version 以规范十进制字符串持久化，并用数字长度加字典序
分页，避免 bigint 上限。Run、Event 与 Receipt 使用 format version 1 的严格编解码；Decimal
保存为无指数规范字符串，任意长度整数按 9 位分块转换，禁止修改进程级整数字符串限制。
PostgreSQL `json` 列写入规范 JSON 文本、读取时显式 cast 为 text，再恢复不可变领域对象；
未知字段、投影不一致、损坏 JSON 或不支持版本均 fail closed 为 `data_corruption`。

所有写入使用短 `READ COMMITTED` 事务且仓储内部不自动重试。提交前且能证明未提交的连接、
锁、超时、死锁或回滚故障映射 `storage_unavailable`；进入 COMMIT 后丢确认，或 SQLSTATE
`08007`/`40003` 映射 `commit_outcome_unknown`。后者只能通过新连接重放原 command 与原
指纹收敛。公共错误和日志不包含 DSN、SQL、参数、payload、最终答案或隐藏推理。

Alembic `alembic_version` 管迁移图，应用自有的 `zhiyi_schema_compatibility` 按组件管理
可接受读写契约，`run_repository=1` 与 `worker_lease_kernel=1` 独立检查并按 engine/component
缓存。迁移只允许独立 Job 执行 `alembic upgrade head`；应用
启动不调用 `create_all`、stamp、upgrade 或修复。首个 downgrade 会逆序删除全部本特性数据，
只允许身份已核验的空库/一次性数据库。保留数据的回退边界是逻辑备份恢复到新数据库后切换，
不是在原生产库执行 downgrade。

PostgreSQL RunRepository 切片不连接 Model Gateway、LangGraph、Tool、Worker、Checkpoint、API
或 SSE；Feature 006 在其上增加独立租约协调内核，但仍不启动或执行这些 Runtime 组件。

## 7. 异步调度与租约

### 7.1 PostgreSQL 队列

Run 记录自身仍是持久队列项，但租约不写回 Run。Feature 006 使用独立
`worker_leases` 保存当前权限事实，使用 `worker_lease_claim_receipts` 保存有界领取重放事实；
前者以 tenant/Run 为主键，保存当前 Worker/claim/token digest、attempt/version 及数据库时间；
后者以 tenant/UUIDv7 claim 为主键，保存版本化意图、24 小时重放期限及完整 claimed/no_work
结果。成功 claimed receipt 只在受限投影中保留回放所需原始 token，token/digest 均不建索引。
Run、Event 和 004 CommandReceipt 契约保持不变。

领取以 `SELECT ... FOR UPDATE SKIP LOCKED` 作为吞吐快路径。快路径准备返回无工作前，必须在
同一短事务内按稳定队头顺序执行一次受有限锁/语句超时约束的阻塞探测；持续外部持锁映射为
`storage_unavailable`，不能持久化为无工作。续租、释放和权限读取使用 PostgreSQL 时间、完整
tenant/Run/Worker/claim/token/attempt/version 证明与单调版本。领取写入使用 Run -> Lease ->
claim receipt 顺序；续租和释放使用 Run -> Lease。Worker 产生的新生命周期写入只能通过更强的
`commit_with_lease` 端口，在同一事务内按 004 收据 -> Run -> Lease -> Event 顺序验证
fencing；普通 005 `commit` 和精确重放语义不变，既有相同 004 收据始终先于租约守卫回放。

续租或释放提交确认未知时，只有 token/attempt/version 未变、状态仍允许且租约未到期，才能
以原条件重试；版本已推进只能确认，证明改变则永久 fence。领取未知只通过原 claim 与原意图
重放；守卫写入未知只通过原 004 命令重放或再次经过当前租约守卫收敛。仓储从不自动重试写入。

Feature 006 只提供持久化协调内核，不启动 Worker、LangGraph、Agent、Tool、Checkpoint 或
Reconciler。调用方只有在当前权限证明为真时才能启动新工作；续租失败或结果未知时必须停止
启动新节点，并按原操作的收敛规则确认事实。
...
Feature 006 已实现一个强制注入的框架无关观测端口。每次公共租约仓储调用在事务、
连接和锁释放后，分别向日志、指标和追踪通道发送一个不可变终态事实；三个通道独立失败，不能
改变业务结果、阻止其他通道或触发数据库重试。事实只含稳定操作名、结果/错误码和有界安全元数据，
不含 token、digest、指纹、DSN、SQL/参数、payload、最终答案或隐藏推理。006 不实现
OpenTelemetry/Langfuse 导出器、后台遥测队列或部署装配。
...
- RunRepository 契约由内存和 PostgreSQL 两个适配器共同覆盖原子提交、幂等、租户隔离、
  分页与并发冲突；真实库另覆盖迁移、重启、100 组/1,000 次竞争、故障窗口和恢复演练。
- LangGraph Checkpoint 和 Interrupt 恢复。
- Tool、MCP、S3 和 Langfuse Adapter。

### 21.3 集成测试

- PostgreSQL 租约并发领取、权限、续租、释放、守卫写入、重启与失效观察已覆盖。
...
### 21.6 PostgreSQL 依赖与发布义务

Feature 005 锁定 SQLAlchemy 2.0.52（MIT）、Alembic 1.19.1（MIT）、Psycopg 与
psycopg-binary 3.3.4（LGPL-3.0-only）；新增传递依赖为 Mako 1.4.1（MIT）、greenlet
3.5.5（MIT AND PSF-2.0）与 typing_extensions 4.16.0（PSF-2.0）。`uv.lock` 是实际依赖
树事实源。发行物必须保留适用许可证/notice；进入预生产前还必须生成 SBOM、执行漏洞与
镜像扫描、复核 psycopg-binary/libpq 的再分发义务，并由发布负责人记录处置。本地测试通过
不等于已经完成生产备份、PITR、Secret、最小权限角色或合规签核。

Feature 006 的 `0002_create_worker_lease_kernel` 是对 0001 的 additive migration；旧 005
应用在 0002 后仍按 `run_repository=1` 工作，新 006 应用独立要求 `worker_lease_kernel=1`。
部署顺序必须是迁移 Job -> 结构/head 检查 -> 新应用 -> 旧连接排空。生产应用角色不得拥有
DDL 权限，迁移角色与应用角色应分离。006 没有 receipt 清理/加密/密钥轮换能力，因此原始
replay token 可能留在数据库和备份；生产启用前还必须建立 PostgreSQL/主机 NTP 偏差与时间
回拨监控。这两项是硬 rollout 阻塞，不由当前通过的迁移或测试替代。
...
Model Gateway、Run Lifecycle Kernel、PostgreSQL RunRepository 与 Worker Lease Kernel
已分别通过其 Spec Kit 实施门禁。后续 M0 切片只有在
以下条件同时满足时才进入编码：
